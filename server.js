...
const getFileInfo = (filePath) => {
  ...
  const match = fileName.match(
    /^(.+?)_(.+?)_(\d{4})\.(\d{2})\.(\d{2})\.(\d{2})\.(\d{2})\.(\d{2})\.jpg$/
  );
  ...
  if (lastUnderscoreIndex !== -1) {
    nome = `${nome} ${restante
      ...
      .replace(/_/g, " ")}`; // Adicionar sobrenome ao nome
  ...
  .on("add", async (filePath) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://pc107662:4002/api/ponto_fotos",
        {
...
app.post("/projetos", (req, res) => {
    ...
    storage: multer.diskStorage({
      destination: (req, file, cb) => {
        try {
          ...
          return cb(
            new Error("Erro ao criar diretório para o upload: " + err.message)
          );
        }
      },
  ...
  upload(req, res, async (err) => {
    ...
    const { NomeProjeto, Empresa, Responsavel, Prazo, EstimativaHoras } =
      req.body;
    ...
    try {
      const estimativaHoras = parseInt(
        EstimativaHoras.replace(/[^\d]/g, ""),
        10
      );
      ...
      res
        .status(201)
        .send({ message: "Projeto criado com sucesso!", id: idProjeto });
    } catch (error) {
...
app.get("/projetos", async (req, res) => {
  try {
    ...
    const result =
      await sql.query`SELECT ID, NomeProjeto, Empresa, Responsavel, Prazo, EstimativaHoras, Layout FROM dbo.projeto`;
    ...
    storage: multer.diskStorage({
      destination: (req, file, cb) => {
        ...
        if (!QualAtividade || !DataDaAtividade) {
          return cb(
            new Error(
              "Os campos QualAtividade e DataDaAtividade são obrigatórios para o upload."
            )
          );
        }
  ...
  upload(req, res, async (err) => {
    ...
    if (!QualAtividade || !DataDaAtividade || !Responsavel) {
      return res.status(400).json({
        error:
          "Campos obrigatórios faltando: QualAtividade, DataDaAtividade ou Responsavel.",
      });
    }
    ...
    try {
      ...
      res
        .status(201)
        .json({ message: "Atividade criada com sucesso", id: idCriado });
    } catch (error) {
...
app.get(
  "/uploads/projeto/:ano/:mes/:dia/:NomeProjeto/:filename",
  async (req, res) => {
    ...
    try {
      ...
      if (ano !== String(anoCriado) || mes !== mesCriado || dia !== diaCriado) {
        return res
          .status(400)
          .json({ error: "Data de criação (Prazo) não corresponde." });
      }
      ...
      console.error(
        `Erro ao acessar o arquivo ou buscar projeto:`,
        error.message
      );
  ...
  try {
    ...
    if (result.recordset.length === 0) {
      return res.status(404).json({
        message: "Nenhuma atividade encontrada para este projeto ou ID.",
      });
    }
...
app.get("/registroDeAtividades/anexos/:id", async (req, res) => {
  ...
  try {
    ...
    if (result.recordset.length === 0) {
      return res
        .status(404)
        .json({ message: "Nenhum anexo encontrado para este ID." });
    }
    ...
    const activityWithAttachments = result.recordset.map((activity) => {
      ...
      const anexosComUrl = anexos
        .map((filePath) => {
          ...
          let relativePath = trimmedFilePath.replace(
            /^.*\\uploads/,
            "/uploads"
          ); // Remove a parte antes de "/uploads"

          // Corrige o caminho da URL, considerando o formato que você mencionou
          let fileUrl = `http://pc107662:4002${relativePath.replace(
            /\\/g,
            "/"
          )}`;
    ...
    res
      .status(500)
      .json({ error: "Erro ao buscar anexos.", details: error.message });
  }
});
...
app.get("/gerar-pdf/:id", async (req, res) => {
  ...
  try {
    ...
    doc
      .fontSize(20)
      .text("Detalhes do Projeto", { align: "center" })
      .moveDown();
    ...
    doc
      .fontSize(14)
      .text("Registros de Atividades", { underline: true })
      .moveDown();
    ...
    atividades.forEach((atividade) => {
      const values = [
        ...
        atividade.HoraInicial
          ? new Date(atividade.HoraInicial).toLocaleTimeString([], {
              hour: "2-digit",
              minute: "2-digit",
            })
          : "N/A",
        atividade.HoraFinal
          ? new Date(atividade.HoraFinal).toLocaleTimeString([], {
              hour: "2-digit",
              minute: "2-digit",
            })
          : "N/A",
      ...
      if (atividade.Anexo && /\.(jpg|jpeg|png)$/i.test(atividade.Anexo)) {
        ...
        if (fs.existsSync(imagePath)) {
          ...
          // Adiciona a imagem
          doc.image(imagePath, startX, currentY, {
            width: imageWidth,
            height: imageHeight,
          });
...
// Endpoint para buscar o status de um projeto pelo ID
app.get("/api/projetos/status/:id", async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    const result = await pool
      .request()
      .input("id", sql.Int, id)
      .query("SELECT id, Status, observacao FROM projeto WHERE id = @id");
